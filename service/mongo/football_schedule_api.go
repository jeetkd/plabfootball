...
func checkSex(url string) (bool, error) {
	...
	if err != nil {
		return false, fmt.Errorf("JSON 데이터 디코딩 오류:", err)
	}
...
func getStadiums(url string) ([]types.StadiumReq, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("JSON 데이터 디코딩 오류:", err)
	}
	return stadiums, nil
}
...
func getBody(url string) ([]byte, error) {
	...
	if resp.StatusCode != http.StatusOK {
		return nil, fmt.Errorf("HTTP 상태 코드 오류:", resp.StatusCode)
	}

	// 응답 본문 읽기
	data, err := ioutil.ReadAll(resp.Body)
	if err != nil {
		return nil, fmt.Errorf("응답 본문 읽기 오류:", err)
	}
	return data, nil
}
